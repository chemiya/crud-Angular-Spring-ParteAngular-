...
export class EditarVinoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.operacion == "editar") { // Si la operacion es editar se captura el id de laURL
      //y se trae el json con el vino, para mostrarlo en el
      //HTML. Si no es “editar”, será “nuevo” y la operacionde
      //traer vino no se realizara y el formulario aparecerávacio
      ...
      this.clienteApiRest.getVino(this.id).subscribe( // Leemos de la base de datos víaAPI
        resp => {
  ...
  onSubmit() {
    ...
    if (this.id) { // si existe id estamos en edicion, si no en añadir
      ...
      this.clienteApiRest.modificarPrecio(String(this.vino.id), this.vino).subscribe(
        resp => {
          if (resp.status < 400) { // Si no hay error en la operacion por parte deservicio
            this.datos.cambiarMostrarMensaje(true);
            this.datos.cambiarMensaje(resp.body); // Mostramos el mensaje enviado por elAPI
          } else {
            this.datos.cambiarMostrarMensaje(true);
      ...
      this.clienteApiRest.anadirVino(this.vino).subscribe(
        resp => {
          if (resp.status < 400) {
            this.datos.cambiarMostrarMensaje(true);
            this.datos.cambiarMensaje(resp.body); // Mostramos el mensaje retornado por elAPI
          } else {
            this.datos.cambiarMostrarMensaje(true);
